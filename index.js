...
try {
  threadStore = JSON.parse(fs.readFileSync(__dirname + "/thread_data.json", "utf-8"));
} catch (err) {
  threadStore = {};
}
try {
  userStore = JSON.parse(fs.readFileSync(__dirname + "/user_data.json", "utf-8"));
} catch (err) {
  userStore = {};
}
...
(async () => {
  ...
  const messageQueue = async.queue((task, callback) => {
    ...
    if (type == "add" && config.StoreMessages) messages_container.addReactions(threadID, item_id, reaction).then(() => callback()).catch(() => callback());
    else if (type == "remove" && config.StoreMessages) messages_container.removeReactions(threadID, item_id, reaction).then(() => callback()).catch(() => callback());
    else if (type == "message_add") { messages_container.add(message); callback(); }
    else if (type == "message_unsend") messages_container.setUnsend({ threadId: threadID, itemId: item_id }).then(() => callback()).catch(() => callback());
  }, 1);

  ig.realtime.on("receive", async (topic, message) => {
    if (config.StoreMessages) ReactionsHandler(message, messageQueue);
  });
  ...
  ig.realtime.on("message", async (message) => {
    try {
      ...
      const context = await ContextHandler.addMessage({ role: "user", content: text }, thread, sender);
      ...
      await ig.entity.directThread(thread).broadcastText(reply);
      await ContextHandler.addMessage({ role: "assistant", content: reply }, thread, sender);
    } catch (err) {
...
function stateExists() {
  return fs.existsSync(__dirname + "/state.json");
}
function loadState() {
  return fs.readFileSync(__dirname + "/state.json", { encoding: "utf-8" });
}
